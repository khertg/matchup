...
2. **Check-in tab:** add players by name and skill level (gender is optional, required for mixed doubles). Returning players auto-complete from the saved roster (IndexedDB). To check in regulars in one go, tick them in **Check in from the roster** (they queue in the order you tick them, with their saved skill and gender) and press one button. In doubles you can lock two players as partners: they share a team and wait in the queue together.
3. **Board tab:** **games never start by themselves.** The **Next up** card shows the next four players (two in singles), already split into Team A and Team B, and the queue marks them too. Press **Start game** on the court that is free to put them on it; Next up then moves on to the following four. Courts show both teams; press "Team A won" or "Team B won" to record a result. Players rejoin the back of the queue and the court stays open until you start the next game. Players on a break are skipped. Undo is available for 10 seconds, until anything else changes. Players watching the live page see the same Next up card. The replace button next to a player swaps in someone waiting. **Add court** opens another court mid-session, and **Manage courts** lets you rename, reorder or close courts. Closing a court with a game in progress asks first, then puts its players back at the front of the queue.
