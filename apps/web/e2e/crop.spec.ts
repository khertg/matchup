...
test.describe('cropping the club logo', () => {
  async function chooseLogo(page: Page) {
    ...
    await dialog.getByLabel('Choose a logo file').setInputFiles({ name: 'logo.png', mimeType: 'image/png', buffer: PICTURE })
    const crop = page.getByRole('dialog', { name: 'Crop club logo' })
    await expect(crop.getByTestId('crop-stage')).toBeVisible()
    return crop
  }
  const logoSrc = (page: Page) => page.getByTestId('club-logo').getAttribute('src')

  test('starts with a rectangle around the whole picture, and using the whole picture keeps it as it is', async ({ page }) => {
    const crop = await chooseLogo(page)
    expect(await cropOf(crop.getByTestId('crop-stage'))).toEqual({ sx: 0, sy: 0, sw: 300, sh: 150 })
    await expect(crop.getByText('Using 300 x 150 of 300 x 150')).toBeVisible()
    await crop.getByRole('button', { name: 'Use whole picture' }).click()
    await expect(page.getByText('Club logo updated')).toBeVisible()
    await page.keyboard.press('Escape')

    const result = await samplePicture(page, (await logoSrc(page))!, [[0.1, 0.5], [0.5, 0.5], [0.9, 0.5]])
    expect([result.width, result.height]).toEqual([256, 128]) // fitted, the same as before cropping existed
    expect(isColor(result.pixels[0], 'red')).toBe(true)
    expect(isColor(result.pixels[1], 'green')).toBe(true)
    expect(isColor(result.pixels[2], 'blue')).toBe(true)
  })

  test('dragging an edge crops the logo to just that part, keeping its shape', async ({ page }) => {
    const crop = await chooseLogo(page)
    const box = crop.getByTestId('crop-stage')
    const west = crop.getByRole('button', { name: 'Resize from the left edge' })
    const handle = (await west.boundingBox())!
    await page.mouse.move(handle.x + handle.width / 2, handle.y + handle.height / 2)
    await page.mouse.down()
    await page.mouse.move(handle.x + handle.width / 2 + 100, handle.y + handle.height / 2, { steps: 6 })
    await page.mouse.up()

    const chosen = await cropOf(box)
    expect(chosen.sx).toBeGreaterThan(60) // moved in from the left by about 100 screen pixels
    expect(chosen.sw).toBe(300 - chosen.sx) // the right edge stayed where it was
    expect([chosen.sy, chosen.sh]).toEqual([0, 150])
    await crop.getByRole('button', { name: 'Crop and use' }).click()
    await expect(page.getByText('Club logo updated')).toBeVisible()
    await page.keyboard.press('Escape')

    const result = await samplePicture(page, (await logoSrc(page))!, [[0.5, 0.5], [0.95, 0.5]])
    // The saved logo is exactly the chosen part (never scaled up), not a square and not the whole picture.
    expect([result.width, result.height]).toEqual([chosen.sw, chosen.sh])
    expect(isColor(result.pixels[1], 'blue')).toBe(true)
  })

  test('dragging the rectangle moves it, and it stays inside the picture', async ({ page }) => {
    const crop = await chooseLogo(page)
    const box = crop.getByTestId('crop-stage')
    const north = crop.getByRole('button', { name: 'Resize from the top edge' })
    const east = crop.getByRole('button', { name: 'Resize from the right edge' })
    const drag1 = async (handle: Locator, dx: number, dy: number) => {
    ...
    await drag1(east, -120, 0) // narrower
    await drag1(north, 0, 40) // shorter
    const shrunk = await cropOf(box)
    expect(shrunk.sw).toBeLessThan(300)
    expect(shrunk.sh).toBeLessThan(150)

    // Move the whole rectangle far to the right and down: it stops at the picture edge.
    const rect = crop.getByTestId('crop-rect')
    const r = (await rect.boundingBox())!
    await page.mouse.move(r.x + r.width / 2, r.y + r.height / 2)
    await page.mouse.down()
    await page.mouse.move(r.x + r.width / 2 + 500, r.y + r.height / 2 + 500, { steps: 8 })
    await page.mouse.up()
    const moved = await cropOf(box)
    expect(moved.sx + moved.sw).toBe(300)
    expect(moved.sy + moved.sh).toBe(150)
    expect([moved.sw, moved.sh]).toEqual([shrunk.sw, shrunk.sh])
  })

  test('the keyboard moves and resizes the rectangle', async ({ page }) => {
    const crop = await chooseLogo(page)
    const box = crop.getByTestId('crop-stage')
    await crop.getByTestId('crop-rect').focus()
    await page.keyboard.press('Shift+ArrowLeft')
    await page.keyboard.press('Shift+ArrowLeft')
    await page.keyboard.press('Shift+ArrowUp')
    const smaller = await cropOf(box)
    expect(smaller.sw).toBeLessThan(300)
    expect(smaller.sh).toBeLessThan(150)
    await page.keyboard.press('ArrowRight')
    await page.keyboard.press('ArrowDown')
    const moved = await cropOf(box)
    expect(moved.sx).toBeGreaterThan(0)
    expect(moved.sy).toBeGreaterThan(0)
    expect([moved.sw, moved.sh]).toEqual([smaller.sw, smaller.sh])
    await expect(crop.getByText(`Using ${moved.sw} x ${moved.sh} of 300 x 150`)).toBeVisible()

    // A handle can be resized from the keyboard too.
    await crop.getByRole('button', { name: 'Resize from the left edge' }).focus()
    await page.keyboard.press('ArrowRight')
    expect((await cropOf(box)).sw).toBeLessThan(moved.sw)
  })

  test('Cancel changes nothing, and the picture never gets smaller than a small minimum', async ({ page }) => {
    const crop = await chooseLogo(page)
    await crop.getByTestId('crop-rect').focus()
    for (let i = 0; i < 120; i++) await page.keyboard.press('Shift+ArrowLeft')
    const tiny = await cropOf(crop.getByTestId('crop-stage'))
    expect(tiny.sw).toBeGreaterThanOrEqual(24)
    await crop.getByRole('button', { name: 'Cancel' }).click()
    const dialog = page.getByRole('dialog', { name: 'Club logo' })
    await expect(dialog.getByText('No logo yet.')).toBeVisible()
    await expect(page.getByTestId('club-logo')).toHaveCount(0)
  })
